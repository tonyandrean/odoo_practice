...
class master_index(models.Model):
    _name='index.index'
    _description='Data index'
    _rec_name='name'
    _sql_constraints = [
        ('name', 'unique(name)', 'index already exist !')
    ]

    name=fields.Char(string='Nama index')

    #hapus data objek
    def unlink(self):
        if self.name == 'tidak terindex':
            raise ValidationError('tidak terindex tidak boleh di hapus')
        res = super(master_index, self).unlink()
        return res

#define class of models survey
class jurnal_internasional(models.Model):
    _name = 'jurnal.internasional'
    _inherit = ['mail.thread']
    _rec_name='isbn'
    _description = 'Jurnal Internasional'
